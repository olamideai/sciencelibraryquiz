...
const quiz = {
    ...
    selectSubject(subject) {
        ...
        const count = questionManager.countBySubject(subject);
        if (count === 0) {
            utils.showToast(`No questions available for ${utils.getSubjectName(subject)}`, 'error');
            return;
        }
    ...
    selectDifficulty(difficulty) {
        ...
        // Check if questions exist for this difficulty
        const count = questionManager.countBySubjectAndDifficulty(this.selectedSubject, difficulty);
        if (count === 0) {
            utils.showToast(`No ${difficulty} questions available for this subject`, 'error');
            return;
        }
        ...
        const availableQuestions = questionManager.countBySubjectAndDifficulty(this.selectedSubject, difficulty);
        const questionsToUse = Math.min(config.questions, availableQuestions);
        
        document.getElementById('quiz-summary').innerHTML = `
            <p><strong>Subject:</strong> ${utils.getSubjectName(this.selectedSubject)}</p>
            <p><strong>Difficulty:</strong> ${difficulty}</p>
            <p><strong>Questions:</strong> ${questionsToUse} (of ${availableQuestions} available)</p>
    ...
    start() {
        ...
        // Get questions from local question manager
        const availableQuestions = questionManager.countBySubjectAndDifficulty(
        ...
        const questionCount = Math.min(config.questions, availableQuestions);
        
        // Get random questions from local bank
        this.questions = questionManager.getRandom(
            this.selectedSubject, 
            this.selectedDifficulty, 
            questionCount
        );
